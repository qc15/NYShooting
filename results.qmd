...
ggplot(crime_data_complete, aes(x = YearMonth, y = IncidentCount, color = BORO, group = BORO)) +
  geom_line(size = 1) +  # Add a line for each BORO
  labs(
...
# Heatmap of Incidents by Time and Borough
```{r}
...
```
The heatmap above shows the number of incidents across different time periods and boroughs. It divides the exact time of shooting incident into 12 periods, each of which consists of 2 hours. From the plot, we can find for all boroughs, the number of incidents increases gradually after 10:00. It reaches the peak in the periods 22:00-00:00 and 0:00-2:00, which is what we expected. The number of incidents decreases after 2:00 and reaches the minimum value in 8:00-10:00. The amount of decrease is more intense from the period 4:00-6:00 to the period 6:00-8:00 compared to changes in other periods. This pattern exists in all boroughs.

```{r}
df_clean_age <- df |>
  filter(VIC_AGE_GROUP != "1022", PERP_AGE_GROUP != "1020", PERP_AGE_GROUP != "(null)", PERP_AGE_GROUP != "", PERP_AGE_GROUP != "940", PERP_AGE_GROUP != "224", PERP_AGE_GROUP != "1028")  # Remove invalid rows
